...
## `PXD034788-human-immunopeptidome.sdrf.tsv`
- parse_sdrf validate-sdrf: `characteristics[sample type]` values are not in the validator's `pride` ontology list

## `PXD034788-mouse-immunopeptidome.sdrf.tsv`
- parse_sdrf validate-sdrf: `characteristics[sample type]` values are not in the validator's `pride` ontology list

Sibling file(s) of this accession that pass the gate are already under `datasets/PXD034788/`: `PXD034788-synthetic.sdrf.tsv`.
